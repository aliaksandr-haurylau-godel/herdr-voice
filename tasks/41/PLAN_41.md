...
**Goal:** `herdr-voice setup` prints the three keybinding blocks this plugin needs,
offers to append them to the user's herdr configuration, appends them without ever
duplicating or shadowing a binding, and names what it changed.

**Architecture:** One new module, `src/setup.rs`. It renders a constant table of
three bindings, reads and parses the user's herdr configuration to decide what is
missing, writes a candidate file, has herdr validate it, and renames it into place.
The interactive half runs in a plugin pane, which has a terminal; the entry point,
which has none, opens that pane and exits.

**Tech Stack:** Rust 2021, `toml` (already a dependency, `Cargo.toml:31`),
`std::io::IsTerminal`. No new dependency.

**Spec:** `tasks/41/DESIGN_41.md`. The criteria it answers are `tasks/41/AC_41.md`.
Read both; every task below argues from them.
...
- Nothing that identifies an employer, a client, an internal system or a private
  machine enters the repository — not in code, comments, examples or test
  fixtures. Paths in examples are generic.
- No panic paths. Every user-visible failure names what to do next.
- Configuration keys have defaults; an absent configuration file is a valid state.
- Device selection by name, never by index. (Untouched here, listed because it is
  a standing rule of the codebase.)
- The four gates pass before every commit: `cargo test`,
  `cargo clippy --all-targets -- -D warnings`, `cargo fmt --check`,
  `python3 scripts/check_manifest.py`.
- Tests must not mutate a process-global environment variable: the suite runs in
  parallel and `src/delivery.rs:171-176` records why. Resolution that reads the
  environment is split into a pure function that takes the values and a thin
  wrapper that reads them.
- The three keys and their actions are fixed by the owner and are not a choice
  this plan may revisit: `ptt` on `ctrl+g`, `dictate` on `prefix+i`, `cancel` on
  `ctrl+shift+g`.

---

### Task 1: The snippet, bound to the manifest by a test

**Files:**
- Create: `src/setup.rs`
- Modify: `src/main.rs` — add `mod setup;` to the module list at `src/main.rs:11-30`

**Interfaces:**
- Consumes: nothing.
- Produces: `setup::PLUGIN_ID: &str`, `setup::Binding { action: &'static str, key:
  &'static str, description: &'static str }`, `setup::BINDINGS: [Binding; 3]`,
  `setup::render(bindings: &[&Binding]) -> String`, and
  `setup::Binding::command(&self) -> String` returning `haurylau.voice.<action>`.
...
mod tests {
    ...
    fn the_three_bindings_are_the_ones_the_owner_chose() {
        let pairs: Vec<(&str, &str)> =
            BINDINGS.iter().map(|b| (b.action, b.key)).collect();
        assert_eq!(
            pairs,
            vec![
                ("ptt", "ctrl+g"),
    ...
    fn every_action_the_snippet_names_is_declared_in_the_manifest() {
        let manifest: toml::Value =
            toml::from_str(&std::fs::read_to_string("herdr-plugin.toml").unwrap())
                .unwrap();
...
Expected: FAIL — `src/setup.rs` does not exist, so the module does not compile.
...
And in `src/main.rs`, beside the other module declarations, `mod setup;`.
...
git commit -m "Render the three keybinding blocks, and bind them to the manifest"
```

---

### Task 2: Which file is written
...
- Produces: `setup::config_path_from(config_path_var: Option<String>, xdg:
  Option<String>, home: Option<String>) -> Option<std::path::PathBuf>` and
  `setup::config_path() -> Option<std::path::PathBuf>`, the thin wrapper that reads
  the three variables from the environment.
...
```rust
    ...
    fn the_override_wins_over_everything() {
        ...
        assert_eq!(p.unwrap(), std::path::Path::new("/tmp/somewhere/other.toml"));
    }
    ...
    fn without_the_override_it_is_herdr_config_toml_under_xdg() {
        ...
        assert_eq!(p.unwrap(), std::path::Path::new("/tmp/xdg/herdr/config.toml"));
    }
...
Run: `cargo test setup::`
Expected: FAIL — `config_path_from` is not defined.
...
git commit -m "Resolve the herdr configuration file the way herdr resolves it"
```

---

### Task 3: What the existing configuration already holds

**Files:**
- Modify: `src/setup.rs`

**Interfaces:**
- Consumes: `setup::PLUGIN_ID` from Task 1.
- Produces: `setup::Existing { commands: Vec<(String, String)>, reserved_keys:
  Vec<String> }` where `commands` is `(key, command)` for every `[[keys.command]]`
  and `reserved_keys` is every value of every string assignment directly under
  `[keys]`; and `setup::inspect(text: &str) -> Result<Existing, String>`.
...
Run: `cargo test setup::`
Expected: FAIL — `inspect` is not defined.
...
pub fn inspect(text: &str) -> Result<Existing, String> {
    ...
    for (name, value) in keys {
        if name == "command" {
            let Some(blocks) = value.as_array() else { continue };
            for block in blocks {
...
```bash
git add src/setup.rs
git commit -m "Read what the herdr configuration already binds"
```

---

### Task 4: Deciding what to add, what is already there, and what is blocked

**Files:**
- Modify: `src/setup.rs`

**Interfaces:**
- Consumes: `setup::BINDINGS`, `setup::Binding` (Task 1); `setup::Existing`
  (Task 3).
- Produces: `setup::Decision { to_add: Vec<&'static Binding>, already: Vec<(&'static
  Binding, String)>, blocked: Vec<(&'static Binding, String)> }` and
  `setup::decide(existing: &Existing) -> Decision`. In `already` the `String` is the
  key our binding already sits on; in `blocked` it is what holds the key.
...
```bash
git add src/setup.rs
git commit -m "Decide what to add, what is present and what is in the way"
```

---

### Task 5: The seam that runs herdr

**Files:**
- Modify: `src/setup.rs`
- Modify: `src/delivery.rs:221-224` — `fn notify_args` becomes `pub(crate) fn
  notify_args`, so `herdr notification show <title> --body <body>` has one
  definition in the codebase rather than two.

**Interfaces:**
- Consumes: `delivery::herdr_binary()`, already `pub` at `src/delivery.rs:141`;
  `delivery::notify_args`, made `pub(crate)` by this task.
- Produces: `setup::HerdrError`, `setup::Check { ok: bool, output: String }`,
  `setup::Herdr` (the trait, with `open_pane`, `check_config`, `notify`),
  `setup::HerdrCli` (the real implementation), and
  `setup::tests_support::FakeHerdr` recording every call.

`trait Deliverer` in `src/delivery.rs` is **not** touched, and no construction of
it changes. Opening a pane and validating a file are not deliveries; the reason is
in `DESIGN_41.md` section 12.
...
```rust
    ...
    #[test]
    fn checking_a_configuration_points_herdr_at_that_file() {
    ...
    fn the_fake_records_what_it_was_asked_to_do() {
        ...
        assert_eq!(
            ...
            vec![
                Call::OpenPane,
                Call::Notify("title".into(), "body".into())
            ]
        );
    }
```
...
Run: `cargo test setup::`
Expected: FAIL — `open_pane_args`, `Check`, `tests_support` are not defined.
...
In `src/delivery.rs`, change the one line:
...
Run: `cargo test setup:: && cargo test delivery::`
Expected: PASS, and `delivery`'s own tests unchanged.

- [ ] **Step 5: Commit**

```bash
git add src/setup.rs src/delivery.rs
git commit -m "Give setup its own seam onto herdr, and share the toast's argv"
```

---

### Task 6: Writing, with herdr validating the candidate first

**Files:**
- Modify: `src/setup.rs`

**Interfaces:**
- Consumes: `setup::Herdr`, `setup::Check`, `setup::HerdrError` (Task 5).
- Produces: `setup::WriteError` and `setup::append(herdr: &dyn Herdr, path: &Path,
  addition: &str) -> Result<(), WriteError>`, which reads the file itself, checks
  the original, writes the candidate beside it, has herdr check that, and renames.
...
```rust
    ...
    fn a_file_that_does_not_exist_is_created_with_its_directory() {
        let path = scratch("absent").parent().unwrap().join("deeper/config.toml");
        append(&FakeHerdr::clean(), &path, "[[keys.command]]\n").unwrap();
        assert!(path.exists());
    }
    ...
    fn a_candidate_herdr_rejects_leaves_the_original_untouched() {
        ...
        let herdr = FakeHerdr::answering(vec![
            Check { ok: true, output: "config: ok".into() },
            Check { ok: false, output: "config: issues found\nctrl+g: disabled".into() },
        ]);
        let err = append(&herdr, &path, "[[keys.command]]\n").unwrap_err();
        ...
        let leftovers: Vec<_> = std::fs::read_dir(path.parent().unwrap())
            ...
            .collect();
        assert_eq!(leftovers.len(), 1, "the candidate must be removed: {leftovers:?}");
    }
...
Run: `cargo test setup::`
Expected: FAIL — `append` and `WriteError` are not defined.
...
pub fn append(herdr: &dyn Herdr, path: &Path, addition: &str) -> Result<(), WriteError> {
    ...
    if let Some(text) = &original {
        let _ = text; // the original exists, so carry its mode onto the candidate
...
```bash
git add src/setup.rs
git commit -m "Write the configuration only when herdr accepts what would be written"
```

---

### Task 7: The run itself, and routing the subcommand

**Files:**
- Modify: `src/setup.rs`
- Modify: `src/main.rs` — the arm at `src/main.rs:174`, the `IMPLEMENTED` list at
  `src/main.rs:119`, the `USAGE` text at `src/main.rs:122-134`, and the test at
  `src/main.rs:220-234` which asserts today that `setup` is *not* implemented.

**Interfaces:**
- Consumes: everything Tasks 1 to 6 produce.
- Produces: `setup::run(herdr: &dyn Herdr, path: Option<PathBuf>, interactive: bool,
  answer: &mut dyn FnMut() -> Option<String>, out: &mut dyn std::io::Write) -> u8`
  and `setup::main() -> u8`, the wrapper `src/main.rs` calls, which resolves the
  path, decides `interactive` from `std::io::stdin().is_terminal()`, and reads the
  answer from standard input.
...
```rust
    ...
    fn a_key_held_by_something_else_is_named_and_the_rest_still_land() {
        ...
        assert_eq!(code, 0, "{said}");
        assert!(said.contains("ctrl+g"), "{said}");
        assert!(said.contains("echo hi"), "it must name what holds the key: {said}");
        let after = std::fs::read_to_string(&path).unwrap();
...
And in `src/main.rs`'s test module, the guard changes rather than being deleted:

```rust
        for name in ["daemon", "doctor", "cancel", "dictate", "model", "ptt", "setup"] {
            assert!(
...
Run: `cargo test`
Expected: FAIL — `run` is not defined, and `main.rs`'s guard fails because `setup`
is not yet in `IMPLEMENTED`.
...
In `src/main.rs`: drop `Command::Setup` from the not-implemented arm, leaving
`other @ (Command::Status | Command::Mic)`, and add

```rust
        Command::Setup => ExitCode::from(setup::main()),
```

beside `Command::Model`. Add `"setup"` to `IMPLEMENTED` at `src/main.rs:119` and a
line to `USAGE`:
...
Run: `cargo test`
Expected: PASS, including `the_usage_text_names_every_implemented_command`.

- [ ] **Step 5: Commit**

```bash
git add src/setup.rs src/main.rs
git commit -m "Run setup: print the snippet, ask, append, and say what changed"
```

---

### Task 8: The pane, the manifest and the README

**Files:**
- Modify: `herdr-plugin.toml` — a `[[panes]]` entry, id `setup`
- Modify: `README.md:44-45`

**Interfaces:**
- Consumes: the `setup` subcommand, routed in Task 7.
- Produces: nothing other code depends on.

- [ ] **Step 1: Add the pane to the manifest**

Beside the other `[[panes]]` entries:
...
The action declared at `herdr-plugin.toml:48-53` stays exactly as it is: it is the
entry point, and what it does now is open this pane.

- [ ] **Step 2: Check the manifest against the binary**

Run: `python3 scripts/check_manifest.py`
Expected: pass — `setup` is already in the subcommand set the script reads out of
`src/main.rs` (it matches the `Some("...")` arms with a whole-file regular
expression rather than a fixed line).

- [ ] **Step 3: Correct the README**

Replace the two sentences at `README.md:44-45`:
...
git commit -m "Declare the setup pane, and say in the README how setup is reached"
```

---

## What is left for a person, and why

Three criteria cannot be met by any test in this plan, and they are verified by
hand and written into `docs/evidence.md` with the platform they were verified on:

1. **AC-3** — that herdr accepts the snippet. Only a real herdr can answer, and
   `herdr config check` is the answer.
2. **AC-4** — that the pane actually opens and takes an answer. The tests drive
   `run` with a fake herdr and a closure for the answer; that the pane appears on
   a screen and a keystroke reaches it is a different claim.
3. That the three keys do what they claim once the configuration is reloaded.

The steps for the person, one at a time and each with what to expect, are written
when the implementation is done — not guessed at now.
