...
pub struct Take {
    ...
    /// The tab that pane sits in, pinned at the same moment as `target`. The
    /// indicator decorates this tab and restores it when the take ends. `None`
    /// means herdr named none.
...
impl Recorder {
    ...
    /// Begins a take for `target`, pinning that pane's agent, working
    /// directory and tab with it.
    pub fn start(
        &self,
        target: &str,
        cwd: Option<&str>,
...
mod tests {
    ...
    fn a_take_started_with_an_agent_reports_it() {
        ...
        assert_eq!(
            recorder.start("w1:p2", None, Some("claude"), None),
            Started::Began
        );
        let take = recorder.stop().expect("a take");
    ...
    fn a_take_reports_the_working_directory_it_was_started_in() {
        ...
        assert_eq!(
            recorder.start("w1:p2", Some("/work/example"), Some("claude"), None),
            Started::Began
        );
        let take = recorder.stop().expect("a take");
    ...
    fn a_device_that_will_not_open_says_so_rather_than_pretending() {
        ...
        match recorder.start("w1:p2", None, None, None) {
